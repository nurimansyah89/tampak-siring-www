...
import { MOCK_RESIDENT_ADMINS } from './resident-admin.model';
...
export class ResidentEditComponent implements OnInit {
  ...
  protected readonly residentId = signal<string | null>(null);
  ...
  ngOnInit(): void {
    ...
    if (id) {
      const resident = MOCK_RESIDENT_ADMINS.find((r) => r.id === id);
      if (resident) {
        this.namaLengkap.set(resident.name);
        this.email.set(resident.email);

        const cleanPhone = resident.phone.replace(/[^0-9]/g, '');
        this.nomorTelepon.set(cleanPhone);

        const blockParts = resident.block.match(/Blok\s+(\w+)/);
        if (blockParts) {
          this.blokProperti.set('Blok ' + blockParts[1]);
        }

        const noParts = resident.block.match(/No\.\s+([\w]+)/);
        if (noParts) {
          this.nomorRumah.set(noParts[1]);
        }
      }
    }
